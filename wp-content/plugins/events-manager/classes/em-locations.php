...
class EM_Locations extends EM_Object {
    ...
    public static function get($args = array(), $count = false) {
        ...
        //       @j@y
        //Filter lookup
        if (!empty($args['em_hike_type'])) {
            $ids = $wpdb->get_col("SELECT post_id FROM " . $wpdb->postmeta . " WHERE meta_key='Hike Type' AND meta_value='{$args['em_hike_type']}'");
            $lids = '';
            if ($ids && is_array($ids)) {
                $pids = implode(",", $ids);
                $conditions['htype_ids'] = "(`$locations_table`.`post_id` IN ($pids))";
            }else {
                $conditions['htype_ids'] = "2=1";
            }
        }
        if (!empty($args['em_hike_difficult'])) {
            $ids = $wpdb->get_col("SELECT post_id FROM " . $wpdb->postmeta . " WHERE meta_key='DIFFICULTY_LEVEL' AND meta_value='{$args['em_hike_difficult']}'");
            $lids = '';
            if ($ids && is_array($ids)) {
                $pids = implode(",", $ids);
                $conditions['hdiff_ids'] = "(`$locations_table`.`post_id` IN ($pids))";
            } else {
                $conditions['hdiff_ids'] = "2=1";
            }
        }
//        @j@y
        //Put it all together
