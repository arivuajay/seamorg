        ...
        if (!empty($_POST['event_rsvp']) && $_POST['event_rsvp']) {
            ...
            if (!$this->is_recurring()) {
                if (get_option('dbem_bookings_tickets_single') && count($this->get_tickets()->tickets) == 1) {
                    ...
                    if (empty($_POST['event_rsvp_date']))
                        $this->event_rsvp_time = $this->event_start_time;
                    if ($this->event_all_day && empty($_POST['event_rsvp_date'])) {
                        $this->event_rsvp_time = '00:00:00';
                    } //all-day events start at 0 hour
...
//                }
        ...
        if (empty($this->event_attributes['MINIMUM_SPACE'])) {
            $this->add_error(sprintf(__("%s is required.", "dbem"), __('Minimum Space', 'dbem')));
        }

        if (!empty($this->event_attributes['MINIMUM_SPACE']) && $this->event_attributes['MINIMUM_SPACE'] > DBEM_CUSTOM_MAX_PERSON_LIMIT) {
            $this->add_error(sprintf(__("Minimum Space should be %d or less.", "dbem"), DBEM_CUSTOM_MAX_PERSON_LIMIT));
        }
                    ...
                    if (EM_MS_GLOBAL && $tax_name == 'category') {
                        ...
                        if (is_array($terms)) {
                            foreach ($terms as $term) {
                                if (!empty($term->slug))
                                    $term_slugs[] = $term->slug; //save of category will soft-fail if slug is empty
                            }
                        }
    ...
    function get_recurrence_days() {
        ...
        switch ($this->recurrence_freq) {
            ...
            case 'yearly':
                ...
                if (@mktime(0, 0, 0, $day, $month, $end_year) < $this->end)
                    $end_year--;
                while ($year <= $end_year) {
