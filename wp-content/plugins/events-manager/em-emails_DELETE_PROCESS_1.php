...
function em_event_submission_emails($result, $EM_Event){
    if( $result ){
		...
		if( $cant_publish_event || $cant_publish_recurring_event ){
		    ...
		    }elseif($EM_Event->is_published() && ($EM_Event->get_previous_status() == 1) ){
//                        When we update
	        	...
	        	if( empty($to_emails) ) return true;
	        	$subject = $EM_Event->output(get_option('dbem_event_updated_email_subject'), 'raw');
	        	$body = $EM_Event->output(get_option('dbem_event_updated_email_body'), $output_type);
...
function em_event_delete_emails($result, $EM_Event) {
    if ($result) {
        if ($EM_Event->is_published() && ($EM_Event->get_previous_status() == 1)) {
//                        When we Delete
            $to_emails[] = $EM_Event->get_contact()->user_email; //send to event owner
            foreach ($EM_Event->bookings->get_user_list() as $bookies):
                $to_emails[] = $bookies->data->user_email;
