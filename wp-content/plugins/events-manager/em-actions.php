...
function em_init_actions() {
    ...
    if (!empty($_REQUEST['em_ajax']) || !empty($_REQUEST['em_ajax_action'])) {
        ...
        if (isset($_REQUEST['em_ajax_action']) && $_REQUEST['em_ajax_action'] == 'get_timeslots') {
            if (isset($_REQUEST['set_date'])) {
                $setDate = $_REQUEST['set_date'];
                $locID = $_REQUEST['location_id'];

                if (isset($_REQUEST['event_id']) && !empty($_REQUEST['event_id'])) {
                    $condition = " AND event_id != '{$_REQUEST['event_id']}'";
                }

                $date = new DateTime($setDate);
                $db_DT = $date->format('Y-m-d');

                $sql = $wpdb->prepare("
					SELECT event_start_time,event_end_time
					FROM " . EM_EVENTS_TABLE . "
					WHERE  event_start_date = '{$db_DT}' AND location_id = '$locID' $condition", $term);
                $results = $wpdb->get_results($sql);

                if ($results) {
                    $slots_of_time = array();
                    foreach ($results as $result) {
                        $slots_of_time[] = date("h:i A", strtotime("$db_DT $result->event_start_time"));
                    }

                    $return = array('status' => true, 'data' => array_unique($slots_of_time));
                    echo EM_Object::json_encode($return);
                    die();
                } else {
                    $return = array('status' => false, 'data' => 'No time lsots in this schedule');
                    echo EM_Object::json_encode($return);
                    die();
                }
            }
