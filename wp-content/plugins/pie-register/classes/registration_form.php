...
class Registration_form extends PieReg_Base
{
	...
	function addPassword($fromwidget)
	{
		...
		$data .= '</div></li><li class="'.$this->pages.' '.$topclass.'"><div class="fieldset"><label>'.$label2.'</label><input id="confirm_password_'.$this->id.'" name="confirm_password" type="password" data-errormessage-value-missing="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-underflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" data-errormessage-range-overflow="'.((isset($this->field['validation_message']))?$this->field['validation_message']:"").'" class="input_fields '.$this->field['css'].' piereg_validate[required,equals['.$this->id.']]" placeholder="Confirm Password" autocomplete="off" />';	
	...
	function addEmail($fromwidget)
	{
		...
		if(isset($this->field['confirm_email']))
		{
			...
			$data .= '</div></li><li class="'.$this->pages.' '.$topclass .'"><div class="fieldset"><label>'.$label2.'</label><input  placeholder="'.$this->field['placeholder'].'" id="confirm_email_'.$this->id.'" '.$this->addValidation().' type="text" class="input_fields '.$this->field['css'].' piereg_validate[required,equals['.$this->id.']]" autocomplete="off">';
		}	
		return $data;
	}
	function addUpload()
	...
	function addName()
	{
		...
		//$data .= '</div></li><li class="'.$this->pages.' '.$topclass.'">';
	...
	function printFields($fromwidget = false)
	{
		...
		if(is_array($this->data)){
			foreach($this->data as $this->field)
			{
				...
				$pie_reg_fields .= '<li class="'.$this->pages.' '.$topclass.'">';
