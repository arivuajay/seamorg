...
class EM_Emails {
    ...
    public static function init() {
        ...
        if (get_option('dbem_cron_feedback', 1)) {
            //set up cron for addint to email queue
            if (!wp_next_scheduled('emp_cron_feedback_queue')) {
                $todays_time_to_run = strtotime(date('Y-m-d', current_time('timestamp')) . ' ' . get_option('dbem_emp_feedback_reminder_time'), current_time('timestamp'));
                $tomorrows_time_to_run = strtotime(date('Y-m-d', current_time('timestamp') + (86400)) . ' ' . get_option('dbem_emp_feedback_reminder_time'), current_time('timestamp'));
                $time = $todays_time_to_run > current_time('timestamp') ? $todays_time_to_run : $tomorrows_time_to_run;
                $time -= ( get_option('gmt_offset') * HOUR_IN_SECONDS ); //offset time to run at UTC time for WP Cron
                $result = wp_schedule_event($time, 'daily', 'emp_cron_feedback_queue');
            }
            add_action('emp_cron_feedback_queue', array('EM_Emails', 'queue_feedback'));
            //set up cron for clearing email queue
            if (!wp_next_scheduled('emp_cron_feedback_process_queue')) {
                $result = wp_schedule_event(time(), 'em_minute', 'emp_cron_feedback_process_queue');
            }
            add_action('emp_cron_feedback_process_queue', array('EM_Emails', 'process_queue'));
        }else {
            //unschedule the crons
            wp_clear_scheduled_hook('emp_cron_feedback_process_queue');
            wp_clear_scheduled_hook('emp_cron_feedback_queue');
        }
    ...
    public static function queue_emails() {
        ...
        foreach (EM_Events::get(array('scope' => $scope, 'private' => 1, 'blog' => get_current_blog_id())) as $EM_Event) {
            ...
            foreach ($EM_Event->get_bookings()->get_bookings()->bookings as $EM_Booking) { //get confirmed bookings
                ...
                if (is_email($EM_Booking->get_person()->user_email)) {
                    ...
                    $emails[] = array($EM_Booking->get_event()->get_contact()->user_email, $subject, $message, $EM_Booking->booking_id);
                }
            }
            if (count($emails) > 0) {
        ...
        public static function queue_feedback() {
        global $EM_Event, $wpdb;
        //save old event in case already set
        $old_EM_Event = !empty($EM_Event) ? clone($EM_Event) : null;
        //disable the current events are past rule
        ...
        foreach (EM_Events::get(array('scope' => $scope, 'private' => 1, 'blog' => get_current_blog_id())) as $EM_Event) {
            ...
            foreach ($EM_Event->get_bookings()->get_bookings()->bookings as $EM_Booking) { //get confirmed bookings
                ...
                $current_user_id = $EM_Booking->person_id;
                $booking_id = $EM_Booking->booking_id;

            $sql = "SELECT COUNT(*) FROM " . $cf7_db_tbl_name . " AS a WHERE submit_time = (SELECT  `submit_time` FROM " . $cf7_db_tbl_name . " AS b WHERE form_name = 'Feedback' AND field_name = 'bookinguserid' AND field_value = '".$current_user_id."' AND a.submit_time = b.submit_time) AND `field_name` = 'bookingid' AND `field_value` = '{$booking_id}'";
            $feedback_state =  $wpdb->get_var($sql);
                /* @var $EM_Booking EM_Booking */
                if ($feedback_state == 0 && is_email($EM_Booking->get_person()->user_email)) {
                    $subject = $EM_Booking->output(get_option('dbem_emp_feedback_reminder_subject'), 'raw');
                    $message = $EM_Booking->output(get_option('dbem_emp_feedback_reminder_body'), $output_type);
                    $emails[] = array($EM_Booking->get_person()->user_email, $subject, $message, $EM_Booking->booking_id);
                }
            }
            if (count($emails) > 0) {
