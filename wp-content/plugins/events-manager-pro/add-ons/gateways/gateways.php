...
if (!class_exists('EM_Gateways')) {
    ...
    class EM_Gateways {
        ...
        static function init() {
            ...
            // Payment return
            add_action('em_payment_return', array('EM_Gateways', 'handle_payment_return'), 10);
        ...
        static function handle_payment_return() {
            if (!empty($_REQUEST['booking_id'])) {
                do_action('em_handle_payment_return_converge');
            }
        ...
        static function get_customer_field($field_name, $EM_Booking = false, $user_or_id = false) {
            ...
            if (get_option('dbem_bookings_registration_disable') && $user_id == get_option('dbem_bookings_registration_user'))
                $user_id = false;
        ...
        static function booking_form_buttons($button = '') {
            ...
            if ($button != '')
                $button .= '<style type="text/css">input.em-booking-submit { display:none; } .em-gateway-button input.em-booking-submit { display:block; }</style>'; //hide normal button if we have buttons
