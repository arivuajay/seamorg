		...
		foreach( $fields as $key => $array ) {
			...
			if ( isset( $array['validate'] ) && !empty( $array['validate'] ) ) {
				...
				switch( $array['validate'] ) {
					...
					case 'alphabetic':
						if ( $args[$key] && !$ultimatemember->validation->is_alphabetic( $args[$key] ) ) {
							$ultimatemember->form->add_error($key, __('Please enter alphabet only in this field','ultimatemember') );
						}
						break;
					case 'numeric':
