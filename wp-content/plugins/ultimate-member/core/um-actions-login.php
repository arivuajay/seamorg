...
function um_add_submit_button_to_login($args) {
    ...
    <div class="um-col-alt">
        <div class="row">
            <div class="um-left col-md-6">
                <?php
                if (isset($args['show_rememberme']) && $args['show_rememberme']) {
                    echo $ultimatemember->fields->checkbox('rememberme', __('Keep me signed in', 'ultimatemember'));
                }
                ?>
            </div>
            <div class="um-right col-md-4 forgot-pass">
                <?php
                if ($args['forgot_pass_link'] != 0) {
                    echo '<a href="' . um_get_core_page('password-reset') . '" class="um-link-alt">Forgot password?</a>';
                }
                ?>
            </div>
        </div>
