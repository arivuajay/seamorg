...
function um_submit_account_errors_hook($args) {
    ...
    } else if ($_POST['um_account_submit'] == 'Update Password') {
        $ultimatemember->form->add_error('current_user_password', __('You must enter your current password', 'ultimatemember'));
        $ultimatemember->account->current_tab = 'password';
    }
...
function um_account_tab__general($info) {
    ...
    if ($output) {
        ...
        <div class="um-col-alt um-col-alt-b">
            <div class="um-center"><input type="submit" name="um_account_submit" id="um_account_submit" value="<?php _e('Update Account', 'ultimatemember'); ?>" class="um-button" /></div>
...
function um_account_user_photo_hook__mobile($args) {
    ...
    <div class="um-account-meta radius-<?php echo um_get_option('profile_photocorner'); ?> uimob340-show uimob500-show">
        ...
        <div class="um-account-meta-img"><a href="<?php echo um_get_core_page('account'); ?>">
                <?php
                if ($user_photo = um_user('photo')) {
                    echo '<img width="120" height="120" alt="" class="gravatar avatar avatar-96 um-avatar" src="' . $user_photo . '">';
                } else {
        ...
        <div class="um-account-name"><a href="<?php echo um_get_core_page('account'); ?>"><?php echo um_user('full_name'); ?></a></div>
...
function um_account_user_photo_hook($args) {
    ...
    <div class="um-account-meta radius-<?php echo um_get_option('profile_photocorner'); ?>">
        ...
        <div class="um-account-meta-img uimob800-hide"><a href="<?php echo um_get_core_page('account'); ?>"><?php
                if ($user_photo = um_user('photo')) {
                    echo '<img width="120" height="120" alt="" class="gravatar avatar avatar-96 um-avatar" src="' . $user_photo . '">';
                } else {
                    echo get_avatar(um_user('ID'), 120);
        ...
        <?php if ($ultimatemember->mobile->isMobile()) { ?>
            ...
            <div class="um-account-meta-img-b uimob800-show" title="<?php echo um_user('display_name'); ?>"><a href="<?php echo um_get_core_page('account'); ?>"><?php
                    if ($user_photo = um_user('photo')) {
                        echo '<img width="120" height="120" alt="" class="gravatar avatar avatar-96 um-avatar" src="' . $user_photo . '">';
                    } else {
                        echo get_avatar(um_user('ID'), 120);
        ...
        <?php } else { ?>
            ...
            <div class="um-account-meta-img-b uimob800-show um-tip-w" title="<?php echo um_user('display_name'); ?>"><a href="<?php echo um_get_core_page('account'); ?>"><?php
                    if ($user_photo = um_user('photo')) {
                        echo '<img width="120" height="120" alt="" class="gravatar avatar avatar-96 um-avatar" src="' . $user_photo . '">';
                    } else {
                        echo get_avatar(um_user('ID'), 120);
        ...
        <div class="um-account-name uimob800-hide"><a href="<?php echo um_get_core_page('account'); ?>"><?php echo um_user('full_name'); ?></a></div>
        ...
        foreach ($ultimatemember->account->tabs as $k => $arr) {
            foreach ($arr as $id => $info) {
                ...
                if (isset($info['custom']) || um_get_option('account_tab_' . $id) == 1 || $id == 'general') {
                    ...
                    <li>
                        <a data-tab="<?php echo $id; ?>" href="<?php echo $url; ?>" class="um-account-link <?php if ($id == $current_tab)
                    echo 'current';
                echo $class;
                ?>">
